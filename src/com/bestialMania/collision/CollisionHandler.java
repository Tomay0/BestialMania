...
public class CollisionHandler {
    ...
    public float getFloorHeightAtLocation(Vector3f position) {
        ...
        for(Triangle triangle : floorAtPoint(position)) {
            float y = triangle.getTriangleY(position, -2.0f,Beast.UPHILL_CLIMB_HEIGHT, MIN_Y);
            if(y>maxY) maxY = y;
        }
        return maxY;
    }
    ...
    public float getCeilingHeightAtLocation(Vector3f position) {
        ...
        for(Triangle triangle : ceilingAtPoint(position)) {
            float y = triangle.getTriangleY(position, -2.0f,Beast.UPHILL_CLIMB_HEIGHT, MAX_Y);
            if(y<minY) minY = y;
        }
        return minY;
    }
